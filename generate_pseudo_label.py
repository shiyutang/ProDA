    ...
    for data_i in tqdm(valid_loader):
        ...
        if opt.soft: # 保存软的伪标签
            ...
            for k in range(labels_val.shape[0]):
                ...
                np.save(os.path.join(ori_LP, name.replace('.png', '.npy')),\
                                         threshold_arg[k].cpu().numpy())
        else:
            if opt.flip:  # 原图预测结果和翻转之后的预测结果的平均
                flip_out = model.BaseNet_DP(fliplr(images_val))
                flip_out['out'] = F.interpolate(sm(flip_out['out']), \
                                    size=images_val.size()[2:], mode='bilinear', align_corners=True)
                out['out'] = F.interpolate(sm(out['out']), size=images_val.size()[2:],\
                                                 mode='bilinear', align_corners=True)
                out['out'] = (out['out'] + fliplr(flip_out['out'])) / 2

            confidence, pseudo = out['out'].max(1, keepdim=True)  # todo: out['out']？
            ...
            for k in range(labels_val.shape[0]):
                name = os.path.basename(filename[k])
                Image.fromarray(pseudo[k,0].cpu().numpy().astype(np.uint8))\
                                    .save(os.path.join(ori_LP, name))
                Image.fromarray(pseudo_rgb[k].permute(1,2,0).cpu().numpy().astype(np.uint8))\
                                    .save(os.path.join(ori_LP, name[:-4] + '_color.png'))
                np.save(os.path.join(ori_LP, name.replace('.png', '_conf.npy')),\
                                     confidence[k, 0].cpu().numpy().astype(np.float16))
                #np.save(os.path.join(ori_LP, name.replace('.png', '_entropy.npy')), \
                #                       entropy[k, 0].cpu().numpy().astype(np.float16))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='./Pseudo', \
...
#python generate_pseudo_label.py --name gta2citylabv2_warmup_soft --soft \
#      --resume_path ./logs/gta2citylabv2_warmup/from_gta5_to_cityscapes_on_deeplabv2_best_model.pkl\
#      --no_droplast
#python generate_pseudo_label.py --name gta2citylabv2_stage1Denoise --flip\
#        --resume_path ./logs/gta2citylabv2_stage1Denoisev2/from_gta5_to_cityscapes_on_deeplabv2_best_model.pkl --no_droplast
...
#  --no_droplast --bn_clr --student_init simclr
#python generate_pseudo_label.py --name syn2citylabv2_warmup_soft 
#                               --soft --src_dataset synthia --n_class 16 
                                # --src_rootpath Dataset/SYNTHIA-RAND-CITYSCAPES 
                                # --resume_path ./logs/syn2citylabv2_warmup/from_synthia_to_cityscapes_on_deeplabv2_best_model.pkl 
                                # --no_droplast